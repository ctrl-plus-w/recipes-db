...
export const getDataOfTable = <TableName extends keyof Database['public']['Tables']>(table: TableName) => {
  return async (rawPage: unknown, rawPerPage: unknown, search?: unknown, column?: keyof Tables<TableName>) => {
    try {
      noStore();
